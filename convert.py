...
parser.add_argument('--text_folder', type=str, help='Input folder of text, labels')
...
#     boxes_folder='/data5/xin/ocr/rot_boxes_v2_class/boxes'
# )
...
def valid(box_file, text_file):
    for decode in ['acsii', 'utf-8', 'latin-1']:
        try:
            box_count = sum([1 for line in open(boxes_path, 'r', encoding=decode) if line.strip()])
            text_count = sum([1 for line in open(text_path, 'r', encoding=decode) if line.strip()])
